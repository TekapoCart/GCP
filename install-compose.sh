...
if [ ! -d "/etc/letsencrypt/live" ]; then
  docker run --rm  -v /etc/letsencrypt:/etc/letsencrypt -p 80:80 -ti certbot/certbot certonly --standalone --email $ADMIN_MAIL --agree-tos --non-interactive --preferred-challenges http -d $TC_DOMAIN
fi
